print("BMMC RNAseq Model Training LR")
...
param_grid = {
    'C': [10**x for x in np.arange(1, 5, 0.5)]
}

log_reg = LogisticRegression() 
gridsearch = GridSearchCV(log_reg, param_grid, n_jobs=-1, cv=cv, verbose=5, return_train_score=True)
...
# save best model
with open('LR_RNAseq_model.pkl','wb') as f:
    pickle.dump(gridsearch.best_estimator_,f)

    
    
with open('LR_RNAseq_model.pkl', 'rb') as f:
